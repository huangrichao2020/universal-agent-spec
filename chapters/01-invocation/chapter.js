  ...
  window.AgentSpec.register({
    ...
    getSvg(lang) {
      ...
      const boxes = phases.map(p => `
        ...
        <text x="${p.x + p.w/2}" y="${p.y + p.h/2 - 7}" text-anchor="middle"
          fill="${p.col}" font-family="'JetBrains Mono',monospace" font-size="12" font-weight="500">${p.top}</text>
        <text x="${p.x + p.w/2}" y="${p.y + p.h/2 + 11}" text-anchor="middle"
          fill="${p.col}99" font-family="'JetBrains Mono',monospace" font-size="9">${p.sub}</text>
      `).join('');

      // Third text line for inference box
      const inferExtra = `
        <text x="265" y="137" text-anchor="middle"
